...
router.get("/nego/:id", (req, res) => {
	util.log("네고 정보 조회");
	getNegoEntry(req.params.id, function (ret) {
		if (ret != null) {
			res.send(ret);
		}
	});
...
// //--- get one data
let getNegoEntry = function (id, callback) {
	util.log("request->" + __API_PRODUCT_URI + "/nego/" + id);
	axios.get(__API_PRODUCT_URI + "/nego/" + id, {
	})
		.then((ret) => {
			util.log("Success to get detail info !");
			let data = ret.data;
